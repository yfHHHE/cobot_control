...
class VideoController:
    ...
    def move_forward(self, distance):
        ...
        target_x = current_coords[0] + distance
        
        # Define parameters for movement control
        speed = 30  # Speed for movement
        tolerance = 1  # Tolerance for reaching target
        
        # Iterate until target position is reached
        while True:
            # Calculate current error
            error = target_x - current_coords[0]
            ...
            move_distance = min(speed, abs(error))  # Move by speed or remaining distance
            move_distance *= -1 if error < 0 else 1  # Move left if error is negative, else move right
            
            # Adjust target coordinates for movement
            target_coords = [current_coords[0] + move_distance, current_coords[1], current_coords[2]]
            
            # Send adjusted target coordinates to the robot
            self.mycobot.send_coords(target_coords, speed, 1)
    ...
    def increaseheight(self, distance):
        ...
        target_z = current_coords[2] + distance
        target_coords = [current_coords[0], current_coords[1], target_z]
        
        for i in range(3):
            if i != 2:  # Skip adjusting Z coordinate (target coordinate)
